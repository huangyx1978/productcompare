...
export class VProductTypeList extends VPage<CDataDictionary>{
    ...
    private renderproducttype=(producttype:any, index:number) => {
        ...
        let right = <div className="row">
                        <div onClick={()=>this.onDisabledClick(producttype)}><FA name={disabled==1?"toggle-off":"toggle-on"} className={classNames(disabled==1?"text-secondary":"text-primary","mx-2")} size="lg" fixWidth={true}/></div>
                        <div onClick={()=>this.onItemClick(producttype)}><FA name="edit" className="text-primary mx-2" size="lg" fixWidth={true}/></div>
                    </div>//列表右侧显示的界面元素
